...
import com.example.a777.card_campus.adapter.DaiwalkAdapter;
...
public class DaiwalkActivity extends AppCompatActivity {
    ListView lv_daiwalks;
    FloatingActionButton sendDaiwalkPost;
    ...
    private Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            DaiwalkAdapter daiwalkadapter = new DaiwalkAdapter(getApplicationContext(),daiwalkResult);
            lv_daiwalks.setAdapter(daiwalkadapter);
            lv_daiwalks.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                    Intent intent = new Intent(DaiwalkActivity.this,DaiwalkDetailActivity.class);
                    intent.putExtra("daiwalk", daiwalkResult.get(i));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_daiwalk);
        getSupportActionBar().hide();

        Button daiwalk_back=(Button)this.findViewById(R.id.daiwalk_back);
    ...
    private void initView() {
        lv_daiwalks = (ListView)this.findViewById(R.id.lv_daiwalks);
        sendDaiwalkPost=(FloatingActionButton)this.findViewById(R.id.daiwalk_send);
        swipeRefreshLayout = (SwipeRefreshLayout)this.findViewById(R.id.refreshlayout_daiwalk);
    }

    private void getDaiwalkList() {
        //实例化OkHttpClient
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if(response.isSuccessful()){
                    ...
                    List<HashMap<String, Object>> daiwalks=null;
                    HashMap<String, Object> daiwalk=null;

                    //从服务器取到Json键值对{“key”:“value”}
                    String temp = response.body().string();
                    try{
                        JSONArray jsonArray=new JSONArray(temp);
                        daiwalks=new ArrayList<HashMap<String, Object>>();

                        for(int i=0;i<jsonArray.length();i++){
                            JSONObject jsonObject=(JSONObject)jsonArray.get(i);
                            daiwalk=new HashMap<String, Object>();

                            daiwalk.put("dpost_id", jsonObject.get("dpost_id"));
                            daiwalk.put("dpost_content",jsonObject.get("dpost_content"));
                            daiwalk.put("dpost_title", jsonObject.get("dpost_title"));
                            daiwalk.put("dpost_time", jsonObject.get("dpost_time"));
                            daiwalk.put("dpost_type", jsonObject.get("dpost_type"));
                            daiwalk.put("is_solved", jsonObject.get("is_solved"));
...
//timeresult是从数据库插到的dpost_time字段，类型为timestamp
                            ...
                            //把时间put进daiactivity
                            daiwalk.put("dpost_time",trueTime);
                            daiwalks.add(daiwalk);
                        }
